...
int main(int argc, char *argv[])
{
	...
	if(rank == 0){
        ...
        cout << "Node[" << name << "]:  Rank " << rank << " finished initialization " << endl;
...
//                                      printf("chunk %i has been sent\n", chunkCount);
                                        ...
                                        cout << "Node " << name <<  " -- Rank " << rank << ": Chunk " << chunkCount << " has been sent, previous chunk processing time: " << duration - oldduration << endl;
...
*/
		...
		cout << endl << endl;
		cout << ColMinReduce << " " << ColMaxReduce << " " << ColSumReduce << " 0 " << ColSumReduce/ColSumReducei << " 0 " << ColSumReducei << " " << timesNoCompareReduce << " " << timesYesCompareReduce << " 0 " << skippedStepsReduce << " " << shiftWasGreaterThan1Reduce << " " << maxSkipStepsReduce << " " << chunkCount << " " << minChunk << " " << maxChunk << " " << avgChunk << " " << avgMin << " " << avgMax << " " << absMin << " " << absMax << endl;
...
//        skippedSteps++;
        ...
        int tempCurr = 1;  // store b_i temporarily for computations after b_i is overwritten; it's 1 because this method should only be used for od$
...
//        }
        ...
        for (; i < lsd + size; i++)
        {
                ...
                num[i+1] = num[i]&1; // get the entry to 0 or 1
                tempCurr = num[i + 1];
        }
