		...
		while(kg){
			...
			if (kg){
				...
				if (chunkCount % 10000 == 0){
                    ...
                    long duration = chrono::duration_cast<chrono::milliseconds>(stopTimerPZero - startTimerPZero).count();

                    cout << "Node " << name <<  " -- Rank " << rank << ": Chunk " << chunkCount << " has been sent, previous chunk processing time: " << duration - oldduration << "\n";
