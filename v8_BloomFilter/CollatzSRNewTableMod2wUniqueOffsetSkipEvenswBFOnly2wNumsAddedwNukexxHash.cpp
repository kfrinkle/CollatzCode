...
public:
	...
	int newlyAddedOnes = 0;
	int allAddedOnes = 0;
	int totalValues = 0;
	int allTotalValues = 0;
	int resets = 0;

	// Note for James: This is a destructor
	~BloomFilter()
	{
		//free(filter);
	}
...
public:
	...
	void addValue(int value[], int binsize, int step)
	{
		...
		value[binsize] = step;
		const string sv = string(reinterpret_cast<const char*>(value), (binsize + 1) * sizeof(int));
		value[binsize] = hold;
		/*uint32_t hash1 = 0;
		uint32_t hash2 = 0;*/
	...
	bool check(int* value, int binsize, int lsd, int cv, int step)
	{
		...
		const string sv = string(reinterpret_cast<const char*>(start), (binsize + 1) * sizeof(int));
...
int CollatzCompareBF(int num[], int sizeNum, int** ColSeq, int steps, int CoalData[], int BinExtra, int ColSeqSizes[], BloomFilter& bf);
...
int main(int argc, char* argv[]) {
	...
	//create bloom filter with size ColSteps
	...
	if (rank > 0) {
		...
		// Thinking about updateTable makes me weep (gonna need to go in and make sure the table itself isn't being built)
