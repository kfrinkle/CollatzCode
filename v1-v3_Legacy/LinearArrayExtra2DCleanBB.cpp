...
*/
	...
	// ok, so let's let everyone catch up here!  If a process(es) got done early, it will wait here.
        cout << "Node " << name <<  " -- Rank " << rank << ": Waiting at the barrier with a streak length of " << streakChunk << endl;

        MPI::COMM_WORLD.Barrier();

        cout << "Node " << name <<  " -- Rank " << rank << ": Sending a streak length value of " << streakChunk << " to Rank 0" << endl;
...
//	now we have to comptue the length of the streak!  So process 0 gathers everyone's streak values.
	...
	if (rank == 0)
	{
		...
		cout << "Node " << name <<  " -- Rank " << rank << ":  Numbers start at (2^" << sizeNum - 1 << ") + 1 and has a run which is " << currrun << " long. Range spanned "  << (long long) size*powa2 << " numbers" << endl;
		if (currrun == (long long int)size * (long long int)powa2)
		{
			cout << "Node " << name <<  " -- Rank " << rank << ": We need to test further!!!!!!" << endl;
		}
