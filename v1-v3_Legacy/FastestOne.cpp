...
void powerfulDiv2Bin(int num[], int size, int shift); // divide by 2 many times in binary (returns new size of binary number)
int mul3p1Bin(int num[], int size); // multiply by 3 and add 1 in binary  (returns new size of binary number)
int mul3p1d2Bin(int num[], int size); // multiply by 3, add 1, then divide by 2 in binary  (returns new size of binary number)
...
//	Now we have each process go through their chunk of the range to check!  Not for loop goes from 0 to powa2-1
	for (i = 0; i < powa2; i++)
	{
        ...
        ii = (long long int)i; // so put number of times through loop (i) into long long int ii for reduce function
...
//	now we have to comptue the length of the streak!  So process 0 gathers everyone's streak values.
	...
	if (rank == 0)
	{
		...
		cout << "Rank " << rank << ":  Numbers start at (2^" << sizeNum - 1 << ") + 1 and has a run which is " << currrun << " long. Range spanned "  << (long long) size*powa2 << " numbers" << endl;
		...
		cout << "Rank " << rank << ": Total times the sizes were unequal (didn't compare digit-by-digit) is :" << timesNoCompareReduce << endl;
		cout << "Rank " << rank << ": Total times the sizes were equal (did compare digit-by-digit) is      :" << timesYesCompareReduce << endl;
		cout << "Rank " << rank << ": Total times we skipped a step is: " << skippedStepsReduce << endl;
...
// ColData[0] holds number of steps to coalesce, [1] and [2] hold min and max coalescence values
	...
	while (binsize > 1)
	{
		...
		if(binnumber[0] == 1)
		{
			binsize = mul3p1d2Bin(binnumber, binsize);
			steps += 2;
		}
		else
		{
		...
		{
			cv = bincompare(binnumber, ColSeq[steps - 1], binsize);
			timesYesCompare++;
		}
		else
		{
...
int bincompare(int num0[], int num1[], int size)
{
	...
	//Since we're only using this method when the sizes are the same, we know that
	//num0[size-1] and num1[size-1] are both 1.
	while (kg && i < size - 1)
	{
		if (num0[i] > num1[i])
		{
...
int mul3p1d2Bin(int num[], int size)
{
        int i = 0;
        ...
        for (i = 0; i < size; i++)
        {
                ...
                carry = num[i]/2; // compute the carry
