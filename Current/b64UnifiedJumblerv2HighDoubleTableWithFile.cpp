...
int main(int argc, char *argv[])
{
	...
	// ARGUEMENTS
	//	std::string file_name = "arguments.txt";
	std::string line;
	std::string filename = argv[1];
	std::ifstream inputFile(filename); // Make this a string variable you place and read it as an argument
	...
	*/
	if (inputFile.is_open())
	{
		...
		{
			arguments_list.push_back(line); // Add the line to the vector
		}
	...
	// Now the arguments_list vector contains all lines as individual "arguments"
	/*
		std::cout << "Arguments list size: " << arguments_list.size() << std::endl;
		for (size_t i = 0; i < arguments_list.size(); ++i)
	{
			std::cout << "Argument " << i + 1 << ": " << arguments_list[i] << std::endl;
		}
	*/

	int expon = std::stoi(arguments_list[0]); // starting exponent of 2
	int powa = std::stoi(arguments_list[1]);  // power of 2 to add to base number for each proccess for the streak (shifted by process)
	// int extra = atoi(argv[3]); // how much padding to add to left end of number -- CHANGE IN COLLATZ FUNCTION TO MATCH
	int tableSampleAmount = std::stoi(arguments_list[2]); // number of integers to build the table with
	vector<Offset<int>> initialOffsets;		   // offset the start of the test range
	vector<Offset<int>> tableThresholdOffsets; // define the range for sampling for the table build
	vector<Offset<int>> testRangeOffsets;
	vector<Offset<unsigned long int>> ulOffsets;
	...
	if (rank == 0)
	{
		std::cout << "Arguments list size: " << arguments_list.size() << " for file: " << filename << std::endl;
		for (size_t i = 0; i < arguments_list.size(); ++i)
		{
			std::cout << "Argument " << i + 1 << ": " << arguments_list[i] << std::endl;
		}
	...
	for (int i = 3; i < arguments_list.size(); i++)
	{
		string tempS(arguments_list[i]);
		...
		if (tempS[0] == 't')
		{
			...
			if (tableThresholdOffsets.size() < 1) throw runtime_error("Need TableThreshold offset!");
		}
		// initial offsets
		else if (tempS[0] == 'o')
		{
		...
		else if (tempS[0] == 'u')
		{
			...
			ulOffsets.push_back(parseULOffset(tempC));
		}
		// skip even toggle
		else if (tempS == "skip")
		{
	...
	for (int i = 0; i < ulOffsets.size(); i++)
	{
		for (unsigned long int j = 0UL; j < ulOffsets[i].multiplier; j++)
		{
			sizeNum = addUL64(num64hold, ulOffsets[i].power, sizeNum);
		}
	}
	...
	*/
	...
	if (rank == 0)
	{
		...
		while (kg)
		{
			...
			if (chunkSize < powa2)
			{
				...
				printf("Rank %i breaks with a chunk size of %i in chunk %ld\n", sender, chunkSizes[sender], chunks[sender]);
				if (!NONSTOP)
				{
	...
	if (rank == 0)
	{
		...
		for (int i = 1; i < size; i++)
		{
			avgMin += minTimes[i];
			avgMax += maxTimes[i];
			if (minTimes[i] < absMin)
				absMin = minTimes[i];
			if (maxTimes[i] > absMax)
				absMax = maxTimes[i];

			if (chunkCounts[i] < minChunk)
				minChunk = chunkCounts[i];
			if (chunkCounts[i] > maxChunk)
				maxChunk = chunkCounts[i];
		...
		*/
		...
		{
			printf(" + ");
			if (initialOffsets[i].multiplier > 1)
				printf("%i(", initialOffsets[i].multiplier);
		...
		if (!bInfos.empty())
		{
			cout << ") with the biggest streak being " << biggestStreak << " long. Range spanned " << (long long int)(chunkCount)*powa2 << " numbers" << endl;
		}
		else
		{
			cout << "), Range spanned " << (long long int)(chunkCount)*powa2 << " numbers with no break found." << endl;
		}
		// now the fun stats!
...
TableBuildInfo updateTable(unsigned long int **ColSeq, int *ColSeqSizes, int ColSteps, int numsize, int startPower, vector<Offset<int>> &initialOffsets, vector<Offset<int>> &tableThresholdOffsets, vector<Offset<unsigned long int>> &ulOffsets, int threshMultiplier, int amountOfSamples, MPI_Comm comm)
{
	...
	struct Sample
	{
		...
		Sample copy(int numsize)
		{
			unsigned long int *arr = new unsigned long int[numsize];
	...
	if (PARALLEL_TABLES)
		MPI_Bcast(intervalOffsets, amountOfSamples, MPI_LONG_LONG, 0, comm);
	...
	for (int i = 0; i < amountOfSamples; i++)
	{
		...
		// add ulOffsets
		for (int j = 0; j < ulOffsets.size(); j++)
		{
			for (unsigned long int k = 0UL; k < ulOffsets[j].multiplier; k++)
			{
				samples[i].size = addUL64(samples[i].num64, ulOffsets[j].power, samples[i].size);
			}
		}

		// add place in the line
	...
	{
		if (rank == 0)
			printf("Attempt to clean index %u, total size of samples is %u\n", dumpIndices.top(), samples.size());
			 ...
			 [](const Sample &a, const Sample &b)
			 {
				 ...
				 if (a.size == b.size)
				 {
					 for (int i = 0; i < a.size + 1; i++)
					 {
						 ...
						 if (a.num64[i] < b.num64[i])
							 return true;
						 else if (a.num64[i] > b.num64[i])
							 return false;
					 }
				 }
...
Offset<unsigned long int> parseULOffset(string &carr)
{
	size_t first = carr.find('[');
	size_t last = carr.find(']');
	Offset<unsigned long int> offset{0UL, 0UL};

	if (first != string::npos && last != string::npos && last > first)
	{
		string multS = carr.substr(0, first);
		string powS = carr.substr(first + 1, last - first - 1);
		try
		{
			offset.multiplier = std::stoul(multS);
			offset.power = std::stoul(powS);
		}
		catch (...)
		{
			cerr << "invalid ul offset: " << carr << "\n";
			offset = {0UL, ULONG_MAX};
		}
	}
	else if (first != string::npos || last != string::npos)
	{
		cerr << "missing a brace on an offset, unable to parse " << carr << "\n";
		offset = {0UL, ULONG_MAX};
	}
	...
	if (offset.power == 0UL)
	{
		cerr << "ul offset " << carr << " is invalid and being ignored." << endl;
		offset.multiplier = 0UL;
	}
	return offset;
}
