...
Offset<unsigned long int> parseULOffset(string carr){
	...
	char* multC = nullptr;
	char* powC = nullptr;
	Offset<unsigned long int> offset;
...
unsigned long int atoul(const char* str){
	...
	for(unsigned int i = 0; i < length; i++){
		...
		unsigned long int hold = output;
		output += static_cast<unsigned long int>(str[(length - 1) - i] - 48) * pow;
